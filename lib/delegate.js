...
module.exports = function delegate(){
  ...
  function delegator( e ){
    ...
    while( ++i < l ){
      ...
      if( matches && !handleNext ) return handleNext
    }
  }

  delegator.add = function( handler ){
    handlers.push([handler])
    return delegator
  }
  delegator.select = function( selector, handler ){
    handlers.push([handler, selector, SELECT])
    return delegator
  }
  ...
  delegator.matches = function( condition, handler ){
    handler.push([handler, condition, SELECT])
  }
  return delegator
}
